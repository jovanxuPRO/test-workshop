...
def gen_code(plan):
    ...
    cf += 'def auth_token():\n'
    cf += '    """Auto-login to get a token if no explicit auth header provided."""\n'
    cf += '    _ah = os.environ.get("TW_AUTH_HEADER", "")\n'
    cf += '    if _ah: return _ah\n'
    cf += '    _u = os.environ.get("TW_LOGIN_USER", "admin")\n'
    cf += '    _p = os.environ.get("TW_LOGIN_PASS", "Admin@123")\n'
    cf += '    try:\n'
    cf += '        r = httpx.post(B + "/api/auth/login", json={"username": _u, "password": _p}, timeout=10)\n'
    cf += '        if r.status_code == 200:\n'
    cf += '            tok = r.json().get("token", "")\n'
    cf += '            if tok: return "Bearer " + tok\n'
    cf += '    except Exception: pass\n'
    cf += '    return ""\n'
    cf += '@pytest.fixture\n'
    cf += 'def c(auth_token):\n'
...
硬性要求: method 字段只能是 GET 或 POST，禁止使用其他方法。
